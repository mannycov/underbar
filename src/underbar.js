(function() {
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection, function(currentValue, currentIndexOrKey, collection) {
      results.push(iterator(currentValue, currentIndexOrKey, collection));
    });
    return results;
  };
  ...
  _.extend = function(obj) {
    _.each(Array.from(arguments).slice(1), function(object) {
  ...
  _.shuffle = function(array) {
    ...
    var swapIndx;
    while(currentIndex) {
      swapIndx = Math.floor(Math.random() * currentIndex);
      currentIndex -= 1;
      temp = out[currentIndex];
      out[currentIndex] = out[swapIndx];
      out[swapIndx] = temp;
    }
    return out;
  };
